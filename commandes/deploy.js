...
✔First type the sc ,repo or script command and you will get elon md repository 
...
✞Elon Md owner , *dullahmd* ,will send you the code immediately.Copy the code and whatsapp will bring a notification 
✞Tap on the notification and paste the code that Elon md owner sent you.
✞ After a successful login Elon Md owner 🤔 dullahmd will send you a *session id* .some will wonder where the hell is that session ,,,...It is just right at your inbox /dm {your own number for whatsappp} 😂
✞Copy the Session id {the one with unreadable codes and long copy it and send it to your deployer or deploy
     𝐇𝐎𝐖 𝐓𝐎 𝐃𝐄𝐏𝐋𝐎𝐘 ELON 𝐌𝐃
✔ Now check on Elon md repository in github fork and give a star to this repository before doing anything 🌟or else elon Md owner won't allow you to deploy his bot🤖.
...
✔Now click on this devs number and give elon md owner credits https://wa.me/qr/2348167574039
╰────────────────────☆`;
let menuMsg = `
     𝐑𝐞𝐠𝐚𝐫𝐝𝐬 ELON MD BOT`;
   var lien = mybotpic();
   ...
   if (lien.match(/\.(mp4|gif)$/i)) {
    try {
        zk.sendMessage(dest, { video: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *dullahmd*, déveloper elon bot" , gifPlayback : true }, { quoted: ms });
    }
    catch (e) {
...
// Vérification pour .jpeg ou .png
    ...
    try {
        zk.sendMessage(dest, { image: { url: lien }, caption:infoMsg + menuMsg, footer: "Je suis *dullahmd*, déveloper elon bot" }, { quoted: ms });
    }
    catch (e) {
        console.log("🥵🥵 Menu erreur " + e);
        repondre("🥵🥵 Menu erreur " + e);
